...
//
// Dart error handlers can't see a native crash — the process (and the Dart
// isolate) is gone before any Dart runs. MetricKit (iOS 14+) records crash
// diagnostics and delivers them to a subscriber on the *next* launch, a few
// seconds in. We persist them to UserDefaults on receipt and let Dart drain
// them via the `getPendingNativeCrashes` method channel call (clears on read
// → each crash reported once).
public class PionnePlugin: NSObject, FlutterPlugin {
  private let subscriber = PionneMetricSubscriber()

  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
      name: "fr.pionne.flutter/native_crash",
      binaryMessenger: registrar.messenger()
    )
    let instance = PionnePlugin()
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
    case "getPendingNativeCrashes":
      result(PionneCrashStore.drain())
    default:
...
enum PionneCrashStore {
  ...
  private static let queue = DispatchQueue(label: "fr.pionne.flutter.crashstore")
