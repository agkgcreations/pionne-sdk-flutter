...
class PionnePlugin : FlutterPlugin, MethodCallHandler {
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        channel = MethodChannel(binding.binaryMessenger, "fr.pionne.flutter/native_crash").also {
        ...
        when (call.method) {
            "getPendingNativeCrashes" -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    result.success(collectPendingCrashes())
                } else {
    ...
    private val prefs
        get() = appContext?.getSharedPreferences("pionne_native", Context.MODE_PRIVATE)
    ...
    private fun mapExit(info: ApplicationExitInfo): Map<String, Any?> {
        ...
        // ANR traces are plain text; native tombstones are protobuf, so only
        // read the trace for ANRs (where it is human-readable).
